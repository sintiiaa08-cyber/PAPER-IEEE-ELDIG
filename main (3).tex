...
% Menggunakan \IEEEoverridecommandlockouts jika Anda perlu mencetak penanda hak cipta di footer
\IEEEoverridecommandlockouts

% PENTING: Paket-paket standar untuk dokumen IEEE
\usepackage{cite} % Untuk sitasi standar IEEE
\usepackage{amsmath,amssymb,amsfonts} % Untuk simbol matematika, terutama untuk notasi bra-ket
\usepackage{graphicx} % Untuk menyertakan gambar
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{array} % Untuk stabilitas tabel di IEEEtran
\usepackage{tikz} % Untuk menggambar diagram Process Flow
\usetikzlibrary{arrows,shapes,positioning} % Library untuk penempatan node TikZ
\usepackage{url} % Untuk URL dalam referensi
\usepackage{multirow} % Untuk stabilitas tabel yang lebih kompleks


% Pengaturan float yang fleksibel
...
\setlength{\textfloatsep}{10pt plus 2pt minus 2pt} % Mengontrol jarak float dari teks
...
% GAMBAR 1 (SETUP FISIK) 
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{placeholder_bottle_filling_setup.png} % Placeholder untuk Gambar Skema Setup
    \caption{Diagram Skematik Sistem Pengisian Botol Otomatis Multi-Sensor}
...
\begin{itemize}
    \item S0 (00): Idle. The system is waiting for a bottle and all actuators are deactivated, except for maintaining pressure (if required).
    \item S1 (01): Positioning. A bottle is detected, and the stepper/servo motors are engaged to precisely align the bottle under the filling nozzle.
    \item S2 (10): Filling. The bottle is aligned, and the solenoid and pump are activated. The PWM/Proportional valve modulates the flow rate based on flow sensor and pressure sensor feedback to ensure optimal filling volume and rate.
\end{itemize}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\columnwidth]{fsm_diagram_bottle_filling.png} 
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{WhatsApp_Image_2025-12-06_at_03.39.11.jpeg} 
...
\begin{enumerate}
    ...
    \item **Positioning $\rightarrow$ Filling Transition:** The input `i_positioned` goes high. At the next clock edge, the state transitions from `01` to `10` (FILLING). The actuator outputs `o_valve_solenoid` and `o_pump_motor` become active, and `o_motor_stepper` deactivates. This confirms accurate sequential control.
    \item **Filling $\rightarrow$ Idle Transition:** The input `i_fill_complete` goes high (from Level Sensor/Load Cell). The state resets to `00` (IDLE) on the next clock edge, deactivating all filling outputs and concluding one complete cycle.
\end{enumerate}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{image_a21424.png} 
...
\begin{enumerate}
    \item **Transition Log Verification:** The log trace clearly shows the state progression (e.g., **Positioning $\rightarrow$ Filling**, **Filling $\rightarrow$ Idle**). This confirms that the software successfully reads the simulated sensor inputs (`[INPUT C] Fill Complete`) and executes the correct transition as defined by the FSM logic.
    \item **Actuator Status Check:** The final output status (`Motor Stepper: OFF`, `Valve Solenoid: OFF`, `Pump Motor: OFF`) in the **Idle** state confirms that the system successfully shuts down all critical actuators after the `Fill Complete` event. This verifies system safety and readiness for the next production cycle.
    \item **Scenario Handling:** The successful execution of test scenarios (Scenario 4 in the image) demonstrates the FSM's ability to handle the nominal operational path, confirming logical compliance and integrity.
\end{enumerate}
